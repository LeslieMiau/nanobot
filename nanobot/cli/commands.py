...
class _GatewayInstanceLock:
    """Best-effort single-instance lock for the active CLI gateway runtime."""
...
def _make_provider(config: Config):
    ...
    if backend == "openai_oauth":
        from nanobot.providers.openai_oauth_provider import OpenAIOAuthProvider
        provider = OpenAIOAuthProvider(default_model=model)
...
def gateway(
    ...
    try:
        ...
        console.print("[red]Another nanobot gateway is already running for this workspace.[/red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        async def watch_coding_tasks() -> None:
            while True:
                for task in coding_task_store.list_tasks():
...
def _login_openai_oauth_family(label: str, provider=None) -> None:
    try:
        ...
        if not (token and token.access):
            ...
            raise typer.Exit(1)
        console.print(f"[green]✓ Authenticated with {label}[/green]  [dim]{token.account_id}[/dim]")
    except ImportError:
...
def _login_openai_codex() -> None:
    _login_openai_oauth_family("OpenAI Codex")
...
def _login_openai_oauth() -> None:
    ...
    _login_openai_oauth_family("OpenAI (OAuth)", provider=OPENAI_OAUTH_PROVIDER)
