...
class ImageGenerateTool(Tool):
    ...
    @property
    def _skip_staging(self) -> bool:
        return getattr(self.config, "skip_staging", False)

    @property
    def description(self) -> str:
        if self._skip_staging:
            return "Generate an image using the configured image model. Always use action='generate'."
        return (
    ...
    def parameters(self) -> dict[str, Any]:
        action_schema: dict[str, Any] = {
            "type": "string",
            "enum": ["generate"] if self._skip_staging else ["stage", "generate"],
            "description": "Generate the image immediately." if self._skip_staging
            else "Whether to stage a prompt for approval or generate the image immediately.",
        }
        props: dict[str, Any] = {
    ...
    async def execute(
        ...
        **_: Any,
        ...
        if getattr(self.config, "watermark", False):
            extra_kwargs["watermark"] = True
